...
fun decimal(digits: List<Int>, base: Int): Int {
    ...
    for (i in digits.indices) {
        k += digits[i] * powInt(base, digits.size - (i + 1))
    }
    return k
}
...
fun russian(n: Int): String {
    ...
    fun rus3(num: Int): String {
        val builderfun = StringBuilder()
        builderfun.append(words3[num / 100] + " ")
        if (num % 100 < 20) builderfun.append(words[num % 100])
        else builderfun.append(words2[(num % 100) / 10] + " " + words[num % 10])
        return builderfun.toString()
    }
    if (n > 999) {
        builder.append(rus3(n / 1000))
        when (if ((n / 1000) % 100 > 19) (n / 1000) % 10 else (n / 1000) % 100) {
